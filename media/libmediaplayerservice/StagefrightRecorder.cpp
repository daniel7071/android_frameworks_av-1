...
status_t StagefrightRecorder::setupVideoEncoder(
    ...
    if (cameraSource != NULL) {
        setupCustomVideoEncoderParams(cameraSource, enc_meta);
    }
    ...
    if (mVideoEncoderProfile != -1) {
        enc_meta->setInt32(kKeyProfile, mVideoEncoderProfile);
    }
    if (mVideoEncoderLevel != -1) {
        enc_meta->setInt32(kKeyLevel, mVideoEncoderLevel);
    }

    enc_meta->setInt32("priority", 0 /* realtime */);
    if (mCaptureFpsEnable) {
        enc_meta->setFloat(kKeyOperatingRate, mCaptureFps);
    }
